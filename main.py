...
def read_msg(user, log_msg, log_time, x):
    ...
    all_msgs = [msg for msg in new_log_msg if '@zapbot' in msg]
    ...
    while True:
        ...
        if (not isEnding(user_response)):
            if (isThanks(user_response)):
                ...
                if greeting(user_response) is None:
...
def isThanks(sentence):
    ...
    return any(word.lower() in agradecimentos for word in sentence.split())

def isEnding(sentence):
    despedidas = ("tchau", "até logo", "fui", "adeus", "até mais")
    return any(word.lower() in despedidas for word in sentence.split())
    ...
    while True:
        ...
        for contato in contatos:    
            try:
                ...
                log_time = [t.text for t in tim_msg]
